...
import { BookingCountdown } from './BookingCountdown.tsx';
...
export const Dashboard = () => {
  ...
  const getBookingDate = (b: any): Date => {
    ...
    if (b.time) {
      const timeMatch = b.time.match(/(\d+)\s*(AM|PM)/i);
      if (timeMatch) {
  ...
  const upcomingBookings = bookings.filter(b => {
    ...
    // Include ongoing or future rituals (up to 2 hours after start)
    return (bDate.getTime() + 2 * 60 * 60 * 1000) >= new Date().getTime();
  });

  const nextBooking = [...upcomingBookings].sort((a, b) => {
    return getBookingDate(a).getTime() - getBookingDate(b).getTime();
  })[0];
                ...
                {activeTab === 'upcoming' && (
                  <div className="space-y-8">
                    ...
                    {loading ? (
                      ...
                      <>
                        <BookingCountdown booking={nextBooking} language={language} />
